...
export const getPodsForResource = (resource: K8sResourceKind, resources: any) => {
...
export const getReplicaSetsForResource = (
  ...
  return sortReplicaSetsByRevision(replicaSets as K8sResourceKind[]).map((rs:K8sResourceKind) =>
    getIdledStatus(toResourceItem(rs, ReplicaSetModel, resources) as PodControllerOverviewItem, deployment),
  );
};

export const getPodsForDeployment = (deployment: any, resources: any) => {
  const obj = {
    ...deployment
  };
